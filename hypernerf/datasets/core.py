...
class DataSource(abc.ABC):
  ...
  def create_iterator(self,
    ...
    return self.iterator_from_dataset(dataset=dataset,
  ...
  def _create_preloaded_dataset(self, item_ids, flatten=False, shuffle=False, ret_dict=False):
    ...
    data_list = [_camera_to_rays_fn(item) for item in tqdm(data_list)]
    # data_list = utils.parallel_map(_camera_to_rays_fn, data_list, show_pbar=True) #[_camera_to_rays_fn(item) for item in tqdm(data_list)]
  ...
  def iterator_from_dataset(self, dataset: tf.data.Dataset,
                          batch_size: int,
                          repeat: bool = True,
                          prefetch_size: int = 0,
                          devices: Optional[Sequence[Any]] = None):
  ...
  def get_item(self, item_id, scale_factor=1.0, do_print=True):
